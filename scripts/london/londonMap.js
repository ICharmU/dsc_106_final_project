async function createLondonGridMap(config) {
  const {
    containerId,
    gridPath,
    wardStatsPath = null,
  ...
  // ---------- 8. Legend (three separate legends, one per layer) ----------
  const legendWidth  = 210;
  const legendHeight = 10;
  const legendMargin = 16;

  // Create a Map to store separate legend SVGs for each layer
  ...
  layerStates.forEach(layer => {
    ...
    const gradient = legendDefs.append("linearGradient")
      .attr("id", `grid-layer-gradient-london-${layer.id}`)
      .attr("x1", "0%").attr("x2", "100%")
      .attr("y1", "0%").attr("y2", "0%");

    // Build gradient based on bins (discrete) or palette (continuous)
    if (layer.bins && layer.colorFn) {
      // Discrete binning: evenly distribute bins across gradient
      const numBins = layer.bins.length;
      layer.bins.forEach((bin, i) => {
        const offset = (i / numBins * 100);
        gradient.append("stop")
          .attr("offset", offset + "%")
          .attr("stop-color", bin.color);
      });
      // Add final stop at 100%
      gradient.append("stop")
        .attr("offset", "100%")
        .attr("stop-color", layer.bins[numBins - 1].color);
    } else {
      // Continuous palette
      const stops = 20;
      const colorScale = d3.scaleSequential(layer.palette)
        .domain([layer.min, layer.max]);
      for (let i = 0; i <= stops; i++) {
        const t = i / stops;
        const v = layer.min + t * (layer.max - layer.min);
        gradient.append("stop")
          .attr("offset", (t * 100) + "%")
    ...
    const legendRect = legendSvg.append("rect")
      ...
      .attr("fill", `url(#grid-layer-gradient-london-${layer.id})`);
    ...
    const legendMinText = legendSvg.append("text")
      ...
      .attr("font-size", 11)
      .attr("fill", "#333")
      .text(layer.min.toFixed(1) + (layer.unit ? " " + layer.unit : ""));
    ...
    const legendMaxText = legendSvg.append("text")
      ...
      .text(layer.max.toFixed(1) + (layer.unit ? " " + layer.unit : ""));

    legendMap.set(layer.id, legendSvg);
  });
  ...
  function updateLayer(animate = false) {
    ...
    legendMap.forEach((legendSvg, layerId) => {
      legendSvg.style("display", layerId === activeLayer.id ? "block" : "none");
    });
  }
...
// ------------------------------------------------------------------
// Concrete instantiations for London
// ------------------------------------------------------------------

// (A) London Greenness section: NDVI-only, no toggle
createLondonGridMap({
  containerId: "#londonNdviMap",
...
}).catch(err => console.error("Error rendering London NDVI map:", err));

// (B) London Greenness vs temperature section: NDVI + LST layers with toggle
createLondonGridMap({
  containerId: "#londonNdviHeatMap",
  gridPath: "data/london/london_grid.json",
  wardStatsPath: "data/london/london_boroughs.json",
  cityName: "London",
  ...
  showLayerToggle: true
}).catch(err => console.error("Error rendering London NDVI/LST map:", err));
