...
export async function createMultiCityGridMap(config) {
  ...
  async function renderCurrentCity(enablelc = false) {
    ...
    if (isCityChanging) {
      ...
      brickOverlay = innerContainer.append("svg")
        .style("position", "absolute")
        .style("top", "0")
        .style("left", "0")
        .style("width", "100%")
    ...
    // Only play animation if city changed
    if (isCityChanging && brickOverlay) {
      ...
      // Get body background color again
      const bodyBgColor = window.getComputedStyle(document.body).backgroundColor;
